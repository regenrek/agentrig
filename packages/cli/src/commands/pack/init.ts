...
const args = {
  name: {
    type: 'positional',
    description: 'Pack name (will create directory)',
    required: true,
  },
  template: {
    type: 'string',
    alias: 't',
    description: 'Template source (default: github:agentrig-ai/pack-starter)',
  },
  dir: {
    type: 'string',
    alias: 'd',
  ...
  title: {
    type: 'string',
    description: 'Pack title (default: derived from name)',
  },
  description: {
    type: 'string',
...
const command = defineCommand({
  meta: {
    name: 'init',
    description: 'Scaffold a new pack from a template.',
  },
  args,
  async run({ args }) {
    if (args.help) return showUsage(command)

    const packName = args.name
    const parentDir = args.dir ? path.resolve(args.dir) : process.cwd()
    const destDir = path.join(parentDir, packName)

    // Validate pack name
    if (!/^[a-z0-9][a-z0-9-]*[a-z0-9]$|^[a-z0-9]$/.test(packName)) {
      console.error(`Invalid pack name: ${packName}`)
      console.error('Pack names must be lowercase, use hyphens, start/end with alphanumeric.')
      process.exit(1)
    }
    ...
    const title = args.title || deriveTitle(packName)
    const description = args.description || `${title} pack for agentrig`
    const author = args.author || ''

    // Substitution map
    const contentSubs: Record<string, string> = {
      '__PACK_NAME__': packName,
      '__PACK_TITLE__': title,
      '__PACK_DESCRIPTION__': description,
      '__PACK_AUTHOR__': author,
    ...
    console.log(`Scaffolding pack "${packName}" from ${templateSpec}...`)
    ...
    if (templateSpec.startsWith('file:') || templateSpec === 'local') {
      ...
      tempDir = await fs.mkdtemp(path.join(tmpdir(), 'agentrig-pack-'))
      try {
        const token = String(
