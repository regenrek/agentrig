...
describe('Agent Plugin package filesystem boundary', () => {
  it('discovers only canonical paths and preserves empty canonical files for semantic validation', async () => {
    ...
    await fs.writeFile(path.join(root, 'scripts', 'server.mjs'), 'process.stdin.resume()\n')
    await fs.writeFile(path.join(root, 'config.json'), '{}\n')
    await fs.writeFile(path.join(root, '.env'), 'SECRET=excluded\n')
    await fs.writeFile(path.join(root, 'CLAUDE.md'), 'provider-owned\n')
    await fs.mkdir(path.join(root, 'node_modules', 'private'), { recursive: true })
    await fs.writeFile(path.join(root, 'node_modules', 'private', 'secret.txt'), 'excluded\n')
    ...
    expect(result.components.supportFiles).toEqual(['config.json', 'scripts/server.mjs'])
