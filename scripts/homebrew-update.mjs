...
const cliPackagePath = path.join(repoRoot, CLI_DIR, "package.json");
...
const tarballUrl = `https://registry.npmjs.org/${pkgName}/-/${pkgBaseName}-${resolvedVersion}.tgz`;
const sha256 = await sha256ForUrl(tarballUrl);

const formulaClass = toFormulaClassName(pkgBaseName);
...
const binName = resolveBinName(pkg.bin, pkgBaseName);
const binRelPath = resolveBinRelPath(pkg.bin, pkgBaseName, binName);
...
const formula = `class ${formulaClass} < Formula
  ...
  def install
    ...
    # npm install doesn't reliably create prefix/bin shims for ESM bins.
...
const formulaPath = path.join(formulaDir, `${pkgBaseName}.rb`);
...
function resolveHomepage(repository) {
  if (!repository) return "https://github.com/regenrek/agentrig";
  const url = typeof repository === "string" ? repository : repository.url;
  if (!url) return "https://github.com/regenrek/agentrig";
...
function resolveBinRelPath(bin, pkgBaseName, binName) {
  if (!bin) return `bin/${pkgBaseName}.mjs`;
